            ...
            if ($count <= 1000) {
                if ($q_1->rowCount() >= 1) {
                    ...
                    if ($p->rowCount() >= 1) {
                        ...
                        $user_rank = $plr['rank'];
                        if ($user_rank == 3) {
                            $all_price = (int) $pd['price_web'] * $count;
                            $price = (int) $pd['price_web'];
                        } else {
                            $all_price = (int) $pd['price'] * $count;
                            $price = (int) $pd['price'];
                        }
                        if ($point >= $all_price) {
                            ...
                            $json_data = json_encode([
                                ...
                                "embeds" => [
                                    [
                                        "title" => "มีการสั่งซื้อสินค้ามาใหม่ [reseller]",
                                        "type" => "rich",
                            ...
                            if ($pd['type'] == "1" and $find_stock->rowCount() >= $count) {
                                ...
                                dd_return(false,  "error", "สินค้าหมด");
                            }
                        } else {
